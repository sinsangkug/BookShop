...
<script>
	...
	//빠른 메뉴의 다음 클릭 시 <hidden>태그에 저장된 상품 정보를 가져와 이미지를 표시 합니다. 
	function fn_show_next_goods(){
		...
		//다음 클릭 시 배열의 인덱스를 1 증가 시킵니다.
		if(array_index <_h_goods_id.length-1)
			array_index++;
		 	
		//증가된 인덱스에 대한 배열 요소의 상품 번호와 이미지 파일 이름을 가져와 표시 합니다.	
...
<body>    
	...
	최근 본 상품 은 상품목록(new ArrayList<GoodsVO>();)에서 상품정보를 가져온  다음 첫 번째 상품 이미지만 표시하고
	다른 상품 이미지는 <hidden>태그에 저장합니다.(동일한  <hidden>태그에 여러개의 데이터 저장 시 자동으로 배열로 저장됩니다.)
	사용자가 다음을 클릭하면 <hidden>태그의 상품 정보를 자바스크립트 함수로 전달하여 이미지를 표시 합니다. 
 ...
 --%>		
	...
	<div class="recent">
		<h3>최근 본 상품</h3>
		  <ul>
		<%--  최근 본 상품 은 상품목록(new ArrayList<GoodsVO>();)에서 상품정보 가  없을 경우 --%>
		 <c:choose>
			<c:when test="${ empty quickGoodsList }">
				     <strong>상품이 없습니다.</strong>
			</c:when>
			
		<%-- 최근 본 상품 은 상품목록(new ArrayList<GoodsVO>();)에서 상품정보 가  있을 경우 --%>
			<c:otherwise>
	        <form name="frm_sticky"  >	        
	       
	       	  <%-- 세션에 저장된 빠른 메뉴 목록의 이미지 정보를 <hidden>태그에 차례대로 저장합니다. --%>
